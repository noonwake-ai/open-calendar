type Int = number
export type NonUndefinedOf<T> = Exclude<T, undefined>
...
declare global {
    interface Array<T> {
        ...
        search<R>(searcher: (ele: T) => R): R | undefined;
...
((proto) => {
        ...
        if (typeof proto.distinctBy !== 'function') {
            proto.distinctBy = function distinctBy<T, U>(this: T[], selector: (e: T) => U): T[] {
                const ret: T[] = []
                for (const e of this) {
                    if (!ret.some(r => selector(r) == selector(e))) {
                        ret.push(e)
                    }
                }
                return ret
        ...
        if (typeof proto.search !== 'function') {
            proto.search = function search<T, R>(this: T[], searcher: (ele: T) => R): R | undefined {
                for (const ele of this) {
                    const ret = searcher(ele)
                    if (ret !== undefined) {
                        return ret
                    }
                }
        ...
        if (typeof proto.takeLast !== 'function') {
            proto.takeLast = function takeLast<T>(this: T[], n: Int): T[] {
                if (n <= 0) return []
